...
void MFRC522::PCD_Init() {
	...
	PCD_WriteRegister(TPrescalerReg, 0x46);		// TPreScaler = TModeReg[3..0]:TPrescalerReg, ie 0x0A9 = 169 => f_timer=40kHz, ie a timer period of 25�s. 0x046 - 10ms
...
MFRC522::StatusCode MFRC522::MIFARE_Ultralight_CheckWrite(	byte page, 		///< The page (2-15) to write to.
														byte *buffer,	///< The 4 bytes to write to the PICC
														byte bufferSize	///< Buffer size, must be at least 4 bytes. Exactly 4 bytes are written.
													) {
	MFRC522::StatusCode result;
	...
	byte buf[18];
    	...
    	byteCount = sizeof(buf);
		result = MIFARE_Read(page, buf, &byteCount);
		if (result != STATUS_OK) {
	        return result;
		}
...
//If the error is greater than one bit an error is returned
if (n>1) return STATUS_ERROR;
        

    
    return STATUS_OK;
} // End MIFARE_Ultralight_CheckWrite()

MFRC522::StatusCode MFRC522::MIFARE_Ultralight_KeyCheck (byte *buffer) {
...
for (byte i=0; i<7; i++){
    ...
    return STATUS_NO_ROOM;
} // End MIFARE_Ultralight_KeyCheck()


void MFRC522::MIFARE_Ultralight_readpage (byte *buffer, byte page) {
...
MFRC522::StatusCode MFRC522::MIFARE_Ultralight_Readtime (byte *times) {
    ...
    else if (buf[2]==buf[10]){
        times[2]=buf[2];
    }
    else if (buf[6]==buf[10]){
        times[2]=buf[6];
    }
    else return STATUS_ERROR;
    ...
    else if (buf[3]==buf[11]){
        times[3]=buf[3];
    }
    else if (buf[7]==buf[11]){
        times[3]=buf[7];
    }
    else return STATUS_ERROR;
...
MFRC522::StatusCode MFRC522::MIFARE_Ultralight_Num (byte *newnum) {
...
/*
Writes to all bits of the memory chip 1 and then 0 with checking  of writing. If there is at least one error, it returns an error.
*/
MFRC522::StatusCode MFRC522::MIFARE_Ultralight_CheckRewrite() {
	MFRC522::StatusCode result;
...
/*
Get to serial UID number of chip with checking
*/


void MFRC522::PICC_DumpMifareUID() {
    MFRC522::StatusCode status;
	byte byteCount;
	byte buffer[18], buffer2[18], buffer3[18];
	    
   	byteCount = sizeof(buffer);
		status = MIFARE_Read(0, buffer, &byteCount);
		if (status != STATUS_OK) {
			tone (3, 2000, 50);
            delay(300);
            tone (3, 2000, 50);
	...
	byteCount = sizeof(buffer2);
		status = MIFARE_Read(0, buffer2, &byteCount);
		if (status != STATUS_OK) {
			tone (3, 2000, 50);
            delay(300);
            tone (3, 2000, 50);
	...
	byteCount = sizeof(buffer3);
		status = MIFARE_Read(0, buffer3, &byteCount);
		if (status != STATUS_OK) {
			tone (3, 2000, 50);
            delay(300);
            tone (3, 2000, 50);
    ...
    for (byte i = 0; i<=6; i++){
        ...
        else if(buffer[i]=buffer3[i]){
            k=buffer[i];
        }
        else if(buffer2[i]=buffer3[i]){
            k=buffer2[i];
        }
        else return;
        Serial.print(k,HEX);
...
}  //End PICC_DumpMifareUltralightToSerial3()

/*)
void MFRC522::PICC_DumpMifareUltralightToSerial4(byte *a) {
    MFRC522::StatusCode status;
	byte byteCount;
	byte buffer[18];
    byte buffer1[18];
    ...
    for (byte page = 0; page < 40; page +=4) { // Read returns data for 4 pages at a time.
		// Read pages with checking up matches in a row twice
		do{
        match = true;
        ...
        byteCount = sizeof(buffer);
		status = MIFARE_Read(page, buffer1, &byteCount);
		if (status != STATUS_OK) {
			tone (3, 2000, 50);
            delay(300);
            tone (3, 2000, 50);
...
/**
 * Reading chip and generation of splits
 */
void MFRC522::PICC_DumpMifareUltralightToSerial5(byte *b1, byte *b2, byte *b3, byte *b4) {
    MFRC522::StatusCode status;
	byte byteCount;
	byte buffer[18];
    byte buffer1[18];
...
void MFRC522::PICC_DumpMifareUltralightToSerial6(byte page, byte *b) {
    MFRC522::StatusCode status;
	byte byteCount;
	byte buffer[18];
    byte buffer1[18];
...
void MFRC522::PICC_DumpMifareUltralightToSerial3() {
    MFRC522::StatusCode status;
	byte byteCount;
	byte buffer[18];
    byte buffer1[18];
...
uint32_t b[36][2]; //decoded array. number of the stantion - time in second
uint8_t cpnum = 0; //number of marks o nthe chip`

//Decoding and forming array of time
uint16_t code1, code2;
bool ok1, ok2;
uint8_t syndrome1, syndrome2;
uint16_t decode1, decode2;
uint32_t d1,d2;

for (byte i = 4; i<=39; i++){
        
    code2 = (a[i][0] << 8) | a[i][1];
    //ok2 = hamming_decode16(&decode2, code2, &syndrome2);
    decode2 = decode2 << 5;
    decode2 = decode2 >> 5;    
    
    code1 = (a[i][2] << 8) | a[i][3];
    //ok1 = hamming_decode16(&decode1, code1, &syndrome1);
    decode1 = decode1 << 5;
    decode1 = decode1 >> 5;    
    
    if ((ok1==0)&&(ok2==0)&&((decode1!=0)||(decode2!=0))) {
        d1=decode1;
        d2=decode2;
        b[cpnum][1] = (d2*2048) +d1;
        b[cpnum][0] = i;
        cpnum++;
...
//Check if competition took place on the border of months
if (b[0][1] < b[1][1]){
    //Searching max full days
    byte fullday = 0;
    for (byte i =1; i<=(cpnum-1);i++){
        if (b[i][1]>b[0][1]){
            if (((b[i][1]-b[0][1])/86400)>fullday) fullday = ((b[i][1]-b[0][1])/86400);
        }
    }
    //to all times less the start time adding fullday in second
    fullday+=b[0][1]/86400;
    for (byte i=1; i<=(cpnum-1);i++){
        if (b[i][1]<b[0][1]){
            b[i][1]+=fullday*86400;
...
//Sorting by time by odd–even sort

uint32_t t1,t2;


for (byte i = 0; i < cpnum; i++) {
	    // (i % 2) ? 0 : 1 It returns 1 if i is even, 0 if i is odd
		for (byte j = (i % 2) ? 0 : 1; j < cpnum - 1; j += 2) {
			if (b[j][1] > b[j + 1][1]) {
				t1 = b[j][0];
                t2 = b[j][1];
                b[j][0]=b[j+1][0];
...
//UID of chip - the first 7 bit in HEX
Serial.print(a[0][0], HEX);
Serial.print(a[0][1], HEX);
Serial.print(a[0][2], HEX);
Serial.print(a[0][3], HEX);
Serial.print(a[1][0], HEX);
Serial.print(a[1][1], HEX);
Serial.print(a[1][2], HEX);
Serial.print(";");

//Total time in second
Serial.print(b[cpnum-1][1]-b[0][1]);
Serial.print(";");

//Number of contorl points
Serial.print(cpnum-2);
Serial.print(";");

//Splits:  second; to cp;
for (byte i = 1; i<=(cpnum-1);i++){
    Serial.print((b[i][1]-b[i-1][1]));
    Serial.print(";");
    Serial.print(b[i][0]);
    Serial.print(";");
}
...
void MFRC522::PICC_DumpMifareUltralightToSerial() {
	...
	for (byte page = 0; page < 230; page +=4) { // Read returns data for 4 pages at a time. ПРАВКА
		// Read pages
		byteCount = sizeof(buffer);
		status = MIFARE_Read(page, buffer, &byteCount);
		if (status != STATUS_OK) {
