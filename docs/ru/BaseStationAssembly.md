...
Инструкция по сборке станции v3 с питанием от 3-х элементов АА (ААА) в корпусе Gainta G1020BF
(архивные инструкции для версий плат [v2](https://github.com/sportiduino/sportiduino/blob/v2.6.3/Doc/ru/BaseStationAssembly.md) и [v1](https://github.com/sportiduino/sportiduino/blob/1.3.7/Doc/ru/BaseStationAssembly.md)).
...
У модуля необходимо выпаять светодиод D1 и резисторы R1 и R2, иначе потребление тока во время его работы будет выше.

Также рекомендуется заменить индуктивности L1 и L2 на аналогичные 2,2 мкГн,
но с большим рабочим током для усиления антенны. Например, Murata LQH32MN2R2K (Спасибо за совет Александру Якимову).
...
В меню Инструменты->Плата выбираем `Sportiduino Base Station`.
...
Подключаем наш программатор к плате Sportiduino и вставляем батарейки в батарейный отсек для более стабильной записи загрузчика и прошивки.
...
Для этого приклеиваем магнит с краю чипа.
...
Если станция прошла проверку, то плату желательно отмыть от остатков флюса изопропиловым спиртом
или специальным средством (например, FluxOff).
