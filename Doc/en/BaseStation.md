...
There is a problem with some clock chips - they start to go badly wrong. To correct this defect, the clock is calibrated. Which adds or subtracts 1 second after the specified period after the last clock correction. It is configured with a master time chip in three bytes. The first byte (0 - set the new time without setting the clock correction, 1 - the hours are late, 2 - the hours are in a hurry, 3 - do not correct the clock), the remaining two bytes are the time in seconds (up to 64 thousand) after which the correction will occur.
