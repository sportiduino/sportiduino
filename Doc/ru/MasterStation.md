...
В прошивке станции сопряжения начиная с версии 1.8.0 реализована частичная поддержка протокола передачи данных системы отметки SPORTident.
Все чипы читаются как SI-Card 6 (192 отметки).
Проверялась работа станции сопряжения с программами SPORTident Reader, Rogain Manager, SportOrg.
С WinOrient пока не работает.
