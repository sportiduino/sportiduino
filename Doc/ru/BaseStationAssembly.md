Фотографии в данной инструкции выполнены для устаревшей версии плат (со старой  пищалкой и без дополнительного транзистора), новая версия отличается незначительно, все операции проделываются также.
...
d. 	Транзистор BC817 sot 23. Или аналогичный.

4.	Пассивные компоненты в формате 0805. 

a.	Резистор 150 Ом. 2 шт

c.	Три резистора 10 кОм

d.	Три конденсатор 0.1 мкФ

e.	Конденсатор 4.7 мкФ. 2 шт

f.      Две индуктивности 2.2 мкГн, >500mA (например LQH32MN2R2K)
...
d.	Пищалка.  стоит заказывать более дорогие и надежные, например,  Jl WorldHC0903A. 
...
Если нет, то где-то допущена ошибка, проверить правильность подключения, пайки и т.п.
...
Мощность антены по-умолчанию 48 дБ, в некоторых станциях данная мощность избыточно и приводит к тому, что чипы перестают отмечаться при поднесении их вплотную. Для станций с этой проблеммой нужно постепенно уменьшать мощность до достижения оптимального срабатывания. Для этого в строке
uint8_t gain нужно уменьшить параметр на единицу (до 0x06 << 4), если срабатывание по-преженему неэффективное, попробовать ещё уменьшить.
