...
При поднесении чипа загорается светодиод и горит или мигает все время процедуры очистки, звуковой сигнал говорит об успешности процедуры.
При поднесении повторно только что очищенного чипа станция просто издает сигнал.
...
С данную инструкцией очень желательно ознакомить участников до начала соревнований.
...
* Оптимальный диапазон срабатывания 0,5 - 1 см. 
...
В релизе присутствует портативный exe файл, не требующий установки.
...
Для быстрой подготовки базовой станции по UART используется преобразователь USB-to-TTL (без подключения DTR).

![](/Images/UsbToTtl.jpg?raw=true)

Подключите его к разъёму Prog базовой станции.
...
Если вы не правильно установите пароль, то базовая станция не примет настройки с мастер-чипа или от преобразователя USB-to-TTL.

![](/Images/SportiduinoPQ-Pwd.jpg?raw=true)

### Мастер-чипы

Другой способ настройки станции — это мастер-чипы.
...
Если вы установите новый пароль и забудете его, то для сброса пароля понадобится перепрошивка станции.
...
Процесс длительный, нужно аккуратно держать чип в оптимальной области записи (0.5 - 1 см над центром белого круга).
